...
- Храните все резюме в начале `com.urise.webapp.storage` (без пустот в виде `null`), чтобы не перебирать каждый раз все 10_000 элементов
...
```
Схема хранения резюме в массиве com.urise.webapp.storage (в элементах от 0 до size-1 отсутствуют null):

r1, r2, r3,..., rn, null, null,..., null
<----- size ----->
<------- com.urise.webapp.storage.length (10000) ------->
```
- Проверьте вашу реализацию с помощью классов `com.urise.webapp.storage.MainArray.main()` и `com.urise.webapp.MainTestArrayStorage.main()`
